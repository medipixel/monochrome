...
이 tool에서는 opensim 모델 동작을 위한 force들에 있는 일종의 noise를 제거합니다. noise 생기는 이유는 여러 가지가 있으나 가장 중요한 요인은 크게 3가지가 있습니다.
- 팔이 없다
- 측정상의 noise
- 측정 위치 마커 에러

이러한 요인들에의해 inverse dynamics를 수행할때 residual force라는 추가적인 힘이 발생하게 됩니다. 그렇기때문에 전체 inverse dynamics를 통해 구하여진 전체 force에서 이 부분 근접하게 산출을 해서 빼줘야 정확한 force를 얻어 낼 수 있습니다. 
